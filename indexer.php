<?php
	...
	function ls($path, $show_folders = false, $show_hidden = false) {
		...
		while (($file = readdir($dh)) !== false) {
			...
			if ($isdir) {
				//uncomment for recursive search. Not finished. Can be full recusrsive.
	...
	if ( !isset($_GET['d']) || substr_count($_GET['d'], '..') > 0 || strpos($_GET['d'], '/') == 0 || !is_dir($_GET['d']) ) 
			$dirTolist = '.';
	else $dirTolist = $_GET['d'];
	...
	// Add parent
	if ($showParent && $dirTolist != '.') array_unshift($items, array(
		'name' => '..',
		'isparent' => true,
		'isdir' => true,
		'size' => 0,
		...
		<ul>
			...
			<?php foreach ($items as $item): ?>
				...
				<li class="item">
					...
					<?php if ($item['isdir']): 
						$currentDir = $item['name'] == '..' ? dirname($dirTolist) : $dirTolist . '/' . htmlentities($item['name']);
					?>
						<a href="<?php echo $_SERVER['PHP_SELF'] . '?d=' . $currentDir; ?>" class="name <?php if ($showIcons) echo $item['isdir'] ? 'directory' : 'file' ?>"><?php echo htmlentities($item['name']) . ($item['isdir'] ? ' /' : '') ?></a>
					<?php else: ?>
